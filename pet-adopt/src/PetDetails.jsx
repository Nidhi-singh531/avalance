...
export default function PetDetails() {
  ...
  useEffect(() => {
    const fetchPet = async () => {
      if (!contract || !petIdParam) return;
      try {
        const data = await contract.getPet(petIdParam);
        setPet({
          name: data[0],
          breed: data[1],
          age: data[2].toNumber(),
  ...
  const foundPet = pets.find(p => p.id === petIdParam);
  ...
  const enhancedPet = {
    ...foundPet,
    description: foundPet.name,
    breed: foundPet.name === 'Mittens' || foundPet.name === 'Luna' || foundPet.name === 'Max' ? 'Asian' : 'Mixed',
    ...
    petId: `TPN${foundPet.id.replace('pet', '')}507`,
